...
def copy_node_to_board(node_id, board_id):
    """
    create a new node based on an existign one from other board
    """
    node = storage.get(CustomNode, node_id)
    new_node = CustomNode()
...
@app_nodes.route('/boards/orphan',
                methods=['DELETE'],
                strict_slashes=False)
def delete_orphans_boards():
    """
    delete all orphan nodes
    """
    nodes = storage.all(Board).values()
    for node in nodes:
        if not node.user_id:
            storage.delete(node)
        else:
            board = storage.get(User, node.user_id)
            if not board:
                storage.delete(node)
    return 'finished'