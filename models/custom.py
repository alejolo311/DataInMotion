...
class CustomNode(BaseNode, Base):
    ...
    def processResponse(self, response):
        ...
        if self.analisis_mode == 'gen_signature':
            ...
            sign = auth.gen_sig(data['key1'],
                                data['key2'], r['data'], url, method)
            print(sign)
            print('====================================')
            return sign
        ...
        if len(json.loads(self.outnodes)) > 0:
            ...
            for nod in outnodes:
                ...
                if node.analisis_mode == 'comparision':
                    ...
                    # print(type(data), data, dir(data), data.get
                    # (list(data.keys())[0]))
                    ...
                    for i, par in enumerate(parms):
                        if 'value1' in par:
                            del parms[i]
                    # ==================================
                    # append the value1 from the first
                    # key in the actual data content
