class WhatsAppRegister extends Component{
	constructor () {
		...
		link.href = `/static/styles/WhatsappRegister.css?${uuid()}`;
		link.rel = 'stylesheet';
		link.setAttribute('name', 'WhatsappRegister');
	...
	async testService (evn, json, phoneNumber) {
		...
		img.style.marginLeft = 'calc(50% - 70px)';
	...
	async register (evn) {
		...
		this._root.querySelector('img').style.borderRadius = '0';
		this._root.querySelector('img').style.width = 300;
		this._root.querySelector('img').style.height = 300;
		this._root.querySelector('img').style.marginLeft = 'calc(50% - 150px)';
		const h1 = this._root.querySelector('h1');
		h1.innerHTML = 'Generating QR code';
		const p = this._root.querySelector('p');
		...
		if (json.url) {
			...
			evn.target.style.visibility = 'visible';
			evn.target.innerHTML = 'test connection'
			const register = this;
